...
class MtgrdleService {
    async startGame(
        ctx: ChatContext<IActionState> | MessageContext<IActionState>
    ) {
        const card = await this.fetchRandomCard(
            ctx.chatInfo,
    ...
    getColorClue(targetColors: string[], guessColors: string[]): string {
        ...
        const commonColors = guessColors.filter((c) =>
            targetColors.includes(c)
        );
    ...
    getManaCostClue(targetManaCost: number, guessManaCost: number): string {
    ...
    getTypeClue(targetTypes: string[], guessTypes: string[]): string {
    ...
    getSetClue(targetCard: CardInfo, guessCard: CardInfo): string {
    ...
    generateClues(targetCard: CardInfo, guessCard: CardInfo): string {
        const clues = [
            this.getSetClue(targetCard, guessCard),
            this.getColorClue(targetCard.colors, guessCard.colors),
            this.getManaCostClue(targetCard.cmc, guessCard.cmc),
            this.getTypeClue(targetCard.types, guessCard.types)
        ];
...
export const mtgrdleService = new MtgrdleService();
